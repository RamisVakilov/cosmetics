...
function cosmetic_scripts() {
	wp_enqueue_style( 'cosmetic-style', get_template_directory_uri().'/asset/css/my.css', array(), _S_VERSION );
	

	wp_enqueue_script( 'cosmetic-navigation', get_template_directory_uri().'/asset/js/script.js', array(), _S_VERSION, true );
...
add_filter('show_admin_bar', '__return_false'); // отключить верхнюю плашку

add_theme_support('menus');//активация меню
 add_filter( 'nav_menu_link_attributes', 'filter_nav_menu_link_attributes', 10, 4 );
 function filter_nav_menu_link_attributes( $atts, $item, $args, $depth) {
 	if($args->menu === 'general'){
	...
	//нахожу ссылки с подменю
	foreach($item->classes as $val){
		if ($val == 'menu-item-has-children'){//класс li у которого есть подменю
			$atts['class']=$atts['class']. ' menu__item-sub';
		}
	}
    if($item->current){//если текущая
		$atts['class'] = $atts['class'] . ' menu__link_active';//добовляем класс активности
		
	}
	 return $atts;
 }
 //добавлене функции редактирования различных полей в теме минуя ACF(В настойке Темы)
